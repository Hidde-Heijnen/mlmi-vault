...
1. **Linear Transformation of Standard Normal Vector:**

   $$
   X = M + L \epsilon, \quad \epsilon \sim \mathcal{N}(0, I_n)
   $$

   - \( L \): A matrix such that \( \Sigma = L L^\top \) (e.g., Cholesky decomposition).
   - \( I_n \): The \( n \times n \) identity matrix.

2. **Equivalent Probability Density Function:**
...
The product of two Gaussian densities yields an unnormalized Gaussian density:
...
- **Combined Mean:**
  ...
  $$
  M = \Sigma \left( \Sigma_1^{-1} M_1 + \Sigma_2^{-1} M_2 \right)
  $$

- **Normalization Constant (\( c \)):** A constant ensuring the equality holds, often omitted when focusing on proportionality.
